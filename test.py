...
def captcha_image():
    # Nếu session đã có CAPTCHA thì giữ nguyên
    if 'captcha_solution' not in session:
        captcha_text = ''.join(random.choices(string.ascii_uppercase + string.digits, k=4))
        session['captcha_solution'] = captcha_text
    else:
        captcha_text = session['captcha_solution']

    if 'captcha_type' not in session:
        session['captcha_type'] = 'captcha1'  # hoặc bất kỳ giá trị nào bạn dùng

    # Tạo ảnh từ CAPTCHA hiện có
...
def view_order():
    ...
    # Validate CAPTCHA
    if 'captcha_solution' not in session or captcha_solution != session['captcha_solution']:
...
@app.route("/")
def index():
    return render_template("index.html", show_captcha=False)
